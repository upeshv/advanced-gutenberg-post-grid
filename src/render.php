 ...
 * 'no_found_rows' bypasses SQL_CALC_FOUND_ROWS for database performance.
 * 'update_post_term_cache' is disabled to save redundant DB queries.
 */
$query_args = array(
    'post_type'              => 'post',
    'post_status'            => 'publish',
    ...
    'update_post_meta_cache' => $display_image, // Only cache meta if we are displaying thumbnails
                        ...
                        <?php if ( true === $display_image && has_post_thumbnail() ) : ?>
                            <div class="advanced-post-grid-preview__image" style="margin-bottom: 15px;">
                                <a href="<?php echo esc_url( get_permalink() ); ?>" aria-hidden="true" tabindex="-1">
                                    <?php
                                    // Injecting our CSS class to enforce aspect-ratio and prevent CLS
                                    the_post_thumbnail(
                                        'medium',
                                        array(
                                            'loading' => 'lazy',
                                            'alt'     => esc_attr( wp_strip_all_tags( get_the_title() ) ),
                                            'class'   => 'apg-placeholder-image', 
                                        )
                                    );
                                    ?>
                                </a>
